...
function updateEmployee() {
    const data = `SELECT employee.first_name AS FirstName, employee.last_name AS LastName FROM employee`
    let updateArray = [];
    ...
    inquirer
        ...
        .then(function (response) {
            // Creates the new role Id for the emplpoyee
            for (let i = 0; i < roleArray.length; i++) {
                roleId = (i + 1);
            }
            data = `INSERT INTO employee (role_id) WHERE employee
            VALUES (${roleId}) WHERE first_name = ${response.employeeName}`
            // add data to the tables
            post(data);
            // Ask main question again
            mainQuestion();
